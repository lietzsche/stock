...
public class DataService {
    ...
    public List<Stock> getCompanyInfo() { // 기본정보 가져오기
        ...
        Document doc = getDocumentByUrl("http://kind.krx.co.kr/corpgeneral/corpList.do?method=download&searchType=13");
    ...
    public Boolean isIdentifier(String code) {
        Document doc = getDocumentByUrl(String.format("https://finance.naver.com/item/main.naver?code=%s", code));
    ...
    public List<Price> getPriceInfo(String code, int page) { // 종목 및 페이지로 가격 정보 가져오기
        Document doc = getDocumentByUrl(
                String.format("http://finance.naver.com/item/sise_day.nhn?code=%s&page=%d", code, page));
