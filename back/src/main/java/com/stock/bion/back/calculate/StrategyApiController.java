...
public class StrategyApiController {
	...
	private List<Stock> fetch(StrategyType type, TimeFrame tf) {
		StrategyResultEntity latest = resultRepository
				.findFirstByStrategyTypeAndTimeFrameOrderByEvaluatedAtDesc(type.name(), tf.name());
		if (latest == null) {
			return List.of();
		}
		LocalDate date = latest.getEvaluatedAt();
		List<StrategyResultEntity> results = resultRepository.findByStrategyTypeAndTimeFrameAndEvaluatedAt(type.name(),
				tf.name(), date);
